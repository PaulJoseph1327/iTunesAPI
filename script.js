...
function displayInfo(res){
    ...
    if(res.resultCount === 0){
        $table.append('<tr class="data"><td colspan="5">NO RESULTS</td></tr>');
    }else {
        for (var i = 0; i < num; i++) {
            ...
            var preview = res.results[i].previewUrl;
            var makeNoise = '<audio controls="true" src="'+ preview +'" id="audio" type="audio/m4a"></audio>';
            var cell = ('<td>' + artwork + '</td><td id="' + i + '"><button class="button" onclick=getDetails(' + i + ')>' + trackName + '</button></td><td>' + artist + '</td><td>' + album + '</td><td>' + makeNoise + '</td>');
